    ...
    static class CalculatorServer extends Server {
        ...
        public Object handleRequest(String method, Object payload) {
            ...
            double result = switch (method) {
                case "addition" -> values.sum();
                case "subtraction" -> values.reduce((left, right) -> left - right).getAsDouble();
                case "multiplication" -> values.reduce((left, right) -> left * right).getAsDouble();
                case "division" -> values.reduce((left, right) -> left / right).getAsDouble();
